---
description: "Test advisor specializing in behavioral coverage and scenario design. Deploys after the developer finishes implementation, in parallel with tester-artisan and tester-architect. Reads the code, reads existing tests, runs them, and produces a Test Advisory focused on what remains untested: missing scenarios, unexplored boundaries, absent regression coverage, unwalked state transitions, uncontested concurrency. Never writes or modifies any file."
mode: "subagent"
hidden: true
color: "success"
tools:
